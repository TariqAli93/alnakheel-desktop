<template>
  <v-container class="pa-4">
    ...
    <v-toolbar rounded="lg" class="mb-4">
      <v-toolbar-title>تحديث عقار</v-toolbar-title>
    </v-toolbar>
    <v-card>
      <v-card-text>
...
// use router to get the property ID from the route
...
const get_proprties_by_id = async () => {
  try {
...
const submitForm = async () => {
  ...
  const modefiedForm = { ...form, elevator: form.elevator * 1 }
  ...
  try {
    const res = await propertyService.update(propertyId, modefiedForm)
